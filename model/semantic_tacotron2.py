...
from utils.utils import graph_to_device
from text.dependency_relations import deprel_labels
...
class Semantic_Tacotron2(nn.Module):
    ...
    def parse_data_batch(self, batch):
        ...
        text, text_length, word_emb, phone2word_idx, g_batch, mel, stop, mel_length = batch
        ...
        return (text, text_length, word_emb, semantic_representation, phone2word_idx, g_batch, mel, mel_length), (mel, stop)

    def forward(self, inputs):
        inputs, input_lengths, word_emb, semantic_representation, phone2word_idx, g_batch, mels, mel_lengths = inputs
        ...
        if self.use_bert:
            if self.use_dependency:
                g_nfeat = torch.reshape(word_emb, [-1, self.bert_dim])
                g_etype = g_batch.edata["type"]
                if self.graph_type.startswith('bi'):
                    word_emb = self.gnn_uni(g_batch, g_nfeat, g_etype) + self.gnn_rev(dgl.reverse(g_batch), g_nfeat, g_etype)
                else:
...
class Semantic_Tacotron2Loss(nn.Module):
    def __init__(self):
        super(Semantic_Tacotron2Loss, self).__init__()
